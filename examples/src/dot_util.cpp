...
dot_util::dot_util(mDot *d) : dot(d)
{
}
...
void dot_util::update_ota_config_name_phrase(std::string network_name, std::string network_passphrase, uint8_t frequency_sub_band, lora::NetworkType network_type, uint8_t ack)
{
...
void dot_util::update_ota_config_id_key(uint8_t *network_id, uint8_t *network_key, uint8_t frequency_sub_band, lora::NetworkType network_type, uint8_t ack)
{
...
void dot_util::update_manual_config(uint8_t *network_address, uint8_t *network_session_key, uint8_t *data_session_key, uint8_t frequency_sub_band, lora::NetworkType network_type, uint8_t ack)
{
...
void dot_util::update_peer_to_peer_config(uint8_t *network_address, uint8_t *network_session_key, uint8_t *data_session_key, uint32_t tx_frequency, uint8_t tx_datarate, uint8_t tx_power)
{
  std::vector<uint8_t> current_network_address = dot->getNetworkAddress();
  std::vector<uint8_t> current_network_session_key = dot->getNetworkSessionKey();
  std::vector<uint8_t> current_data_session_key = dot->getDataSessionKey();
...
void dot_util::update_network_link_check_config(uint8_t link_check_count, uint8_t link_check_threshold)
{
...
void dot_util::sleep_wake_rtc_only(bool deepsleep)
{
  // in some frequency bands we need to wait until another channel is available before transmitting again
...
void dot_util::sleep_wake_interrupt_only(bool deepsleep)
{
...
void dot_util::sleep_wake_rtc_or_interrupt(bool deepsleep)
{
  // in some frequency bands we need to wait until another channel is available before transmitting again
  // wait at least 10s between transmissions
  uint32_t delay_s = dot->getNextTxMs() / 1000;
...
void dot_util::sleep_configure_io()
{
#if defined(TARGET_XDOT_L151CC)
...
void dot_util::sleep_restore_io()
{
#if defined(TARGET_XDOT_L151CC)
...
int dot_util::send_data(std::vector<uint8_t> data)
{
  int32_t ret;
